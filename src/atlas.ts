...
interface MapData {
  [name: string]: { xPos: number; yPos: number; width: number; height: number };
}
...
async function createTextureAtlas({
  ...
  if (input) {
    if (fs.existsSync(input)) {
      if (input.indexOf(":") >= 0) textureFolder = input;
      else textureFolder = path.join(_dir, input);
    } else {
      console.error(`Could not find directory '${input}'.`);
      return;
    }
  } else textureFolder = _dir;

  let outputFolder: string;
  if (output) {
    if (fs.existsSync(output)) {
      if (output.indexOf(":") >= 0) outputFolder = output;
      else outputFolder = path.join(_dir, output);
    } else {
      if (output.indexOf(":") >= 0) {
        fs.mkdirSync(output);
        outputFolder = output;
